...
export default class SizeAndPositionManager {
	 ...
	 * @param {number} offset
	 * @param {number|null} overscanCount
	...
	getVisibleRange({ containerSize = 0, offset, overscanCount }) {
		...
		// console.log({ containerSize, overscanCount, offset });
		...
		// console.log('datum.offset + datum.size', datum.offset + datum.size);
	...
	getElementVisibleHeight({ ele, container }) {
		if (!ele || !container) return 0;
		const { top: eleTop, bottom: eleBottom } = ele.getBoundingClientRect();
		const { top: containerTop, bottom: containerBottom } = container.getBoundingClientRect();

		// Determine the top and bottom bounds of the visible portion
		const visibleTop = Math.max(eleTop, containerTop);
		const visibleBottom = Math.min(eleBottom, containerBottom);

		// Calculate the visible height (if negative, it means no intersection)
