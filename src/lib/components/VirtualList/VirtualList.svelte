...
<script>
	...
	export let overscanCount = null;
	/**
	 * @type {HTMLElement | null}
	 */
	export let wrapper = null;
	let hasExternalWrapper = false;
	...
	function refresh() {
		...
		let visibleSize = sizeAndPositionManager.getElementVisibleHeight({
			ele: root,
			container: wrapper
		});

		// overscanCount = Math.floor(visibleSize / itemSize);
		// console.log({ overscanCount });

		const { start, stop } = sizeAndPositionManager.getVisibleRange({
			// containerSize: isContainerVisible ? visibleSize : visibleSize - visibleScrollSize,
			// containerSize: 0,
	...
	function getOffsetForIndex(index, align = scrollToAlignment, _itemCount = itemCount) {
		...
		const visibleSize = sizeAndPositionManager.getElementVisibleHeight({
			ele: root,
			container: wrapper
		});
	...
	function getStyle(index, sticky) {
		...
		if (scrollDirection === DIRECTION.VERTICAL) {
			...
			if (sticky) {
				style += `position:sticky;flex-grow:0;z-index:99;top:0;margin-top:${offset}px;margin-bottom:${-(offset + size)}px;`;
			} else {
...
<div bind:this={root} class="virtual-list-wrapper" style={wrapperStyle}>
	<slot name="header" />
